if SERVER then
	...
	hook.Add('HUDPaint', 'BGN_DrawWantedText', function()
		...
		local timeleft = bgNPC:GetEntityVariable(LocalPlayer(), 'wanted_time', GetConVar('bgn_wanted_time'):GetFloat())
	...
	hook.Add('PreDrawHalos', 'BGN_RenderOutlineOnNPCCallingPolice', function()
		...
		for _, actor in ipairs(bgNPC:GetAll()) do
			local npc = actor:GetNPC()
			if IsValid(npc) and actor:GetState() == 'calling_police' and npc:GetPos():DistToSqr(LocalPlayer():GetPos()) < 6250000 then
	...
	hook.Add('PostDrawOpaqueRenderables', 'BGN_RenderTextAboveNPCCallingPolice', function()
		for _, actor in ipairs(bgNPC:GetAll()) do
			local npc = actor:GetNPC()
			if IsValid(npc) and actor:GetState() == 'calling_police' and npc:GetPos():DistToSqr(LocalPlayer():GetPos()) < 6250000 then -- 2500 ^ 2
