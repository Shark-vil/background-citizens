timer.Create('BGN_FixNPCRelationship', 1, 0, function()
	...
	for _, ActorOne in ipairs(actors) do
		...
		for _, ActorTwo in ipairs(actors) do
			if ActorOne == ActorTwo then continue end
			local npc_2 = ActorTwo:GetNPC()

			if IsValid(npc_1) and IsValid(npc_2)
				and ActorOne:HasTeam(ActorTwo)
				and npc_1:Disposition(npc_2) ~= D_NU
				or npc_2:Disposition(npc_1) ~= D_NU then
