...
hook.Add('BGN_NPCLookAtObject', 'BGN_NPCDoorOpeningEvent', function(actor, door)
	...
	if not (not door.bgNPCOpenDoor and hook.Run('BGN_PreOpenDoor', actor, door) == nil) then return end
	...
	timer.Simple(10, function()
		if not (IsValid(door) and door.bgNPCOpenDoor and not hook.Run('BGN_PreCloseDoor', door)) then return end
