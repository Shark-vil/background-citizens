...
function BG_NPC_CLASS:Instance(npc, type, data)
	...
	function obj:GetNearTarget()
		...
		for _, npcFor in ipairs(self.targets) do
			if not IsValid(npcFor) then continue end

			if not IsValid(target) then
				target = npcFor
				dist = npc:GetPos():DistToSqr(self_npc:GetPos())
			elseif npcFor:GetPos():DistToSqr(self_npc:GetPos()) < dist then
				target = npcFor
				dist = npcFor:GetPos():DistToSqr(self_npc:GetPos())
			end
		end
	...
	function obj:SetOldState()
		...
		if not self.old_state then return end

		self.npc.bgNPCtate = self.old_state
		self.old_state = nil

		if IsValid(self.npc) then
			hook.Run('BGN_SetNPCState', self, self.npc.bgNPCtate.state, self.npc.bgNPCtate.data)
		end
	end

	function obj:SetState(state, dataState)
		if self:GetData().disableStates then return end
		if self.state_lock then return end
		...
		if SERVER and (self.npc.bgNPCtate == nil or self.npc.bgNPCtate.state ~= state) and math.random(0, 10) <= 1 then
		...
		self.npc.bgNPCtate = { state = state, dataState = dataState or {} }
	...
	function obj:HasTeam(team_value)
		if not (self.data.team ~= nil and team_value ~= nil) then return false end
	...
	function obj:UpdateStateData(dataState)
		self.npc.bgNPCtate.data = dataState
	end
	...
	function obj:GetState()
		return self.npc.bgNPCtate and self.npc.bgNPCtate.state or 'none'
	end

	function obj:GetStateData()
		return self.npc.bgNPCtate and self.npc.bgNPCtate.data or {}
	end
	...
	function obj:GetDistantPointInRadius(pos, radius)
		...
		local point = nil
		local dist = 0
		local _npc = self:GetNPC()
		local points = bgNPC:GetAllPointsInRadius(_npc:GetPos(), radius)
