if CLIENT then
	...
	function bgNPC:SpawnActor(type)
		local bgn_spawn_radius = GetConVar('bgn_spawn_radius'):GetFloat()
		...
		for id, data in ipairs(bgNPC.npc_classes) do
			...
			do
				...
				if IsValid(ply) then
					points_close = bgNPC:GetAllPointsInRadius(ply:GetPos(), bgn_spawn_radius)
				end
			end
			...
			for _, ply in ipairs(players) do
				...
				if not (distance < bgn_spawn_radius_visibility and bgNPC:PlayerIsViewVector(ply, pos)) then continue end

				if distance <= bgn_spawn_block_radius or (bgn_spawn_radius_raytracing ~= 0 and bgn_spawn_radius_raytracing < distance) then
					return
				end
